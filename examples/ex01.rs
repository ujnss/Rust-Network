...
fn test01(io: &mut NetIO) {
  ...
  if partyid == 0 {
    io.send(1, msgid.clone(), &data);
    let d = io.recv(2, msgid.clone());
    info!("recv from 2 data {:?}", d);
  }
  if partyid == 1 {
    let d = io.recv(0, msgid.clone());
    info!("recv from 0 data {:?}", d);
    io.send(2, msgid.clone(), &data);
  }
  if partyid == 2 {
    let d = io.recv(1, msgid.clone());
    info!("recv from 1 data {:?}", d);
    io.send(0, msgid.clone(), &data);
  }
}
...
fn main() {
  ...
  // New a NetIO
  let mut io = NetIO::new(partyid, &participants);
