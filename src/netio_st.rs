...
impl NetIOST {
  ...
  //!
  //! let mut io = NetIOST::new(partyid, &participants).expect("new NetIOST");
  //! io.stop();
  //! ```
  //!
  ...
  /// New a NetIOST with self `partyid` and the `participants`.
